...
export default function GetSongs() {
	...
	return (
		<View style={styles.container}>
			<Button title={loading ? 'Loading...' : 'Load All Songs'} onPress={getAllMedia} disabled={loading} />
			...
			<FlatList
				...
				renderItem={({ item }) => (
					...
					>
						<Image
							source={item.artwork ? { uri: item.artwork } : require('@/assets/images/react-logo.png')}
							style={{
								width: 60,
								height: 60,
								borderRadius: 8,
						...
						<View style={{ flex: 1 }}>
							<Text style={{ fontSize: 16, fontWeight: 'bold' }}>{item.title ?? item.filename}</Text>
							<Text style={{ fontSize: 13, color: '#666' }}>{item.artist ?? 'Unknown Artist'}</Text>
						</View>
					</View>
				)}
			/>
